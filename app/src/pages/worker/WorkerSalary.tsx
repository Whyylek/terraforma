...
import { mockWorkLogs, mockSalaryRecords } from '@/data/mockData';
...
export default function WorkerSalary() {
  ...
  const workerSalaries = mockSalaryRecords.filter(s => s.worker_id === user?.id);
  
  const hourlyRate = user?.salary_hourly_rate || 0;
  const kmRate = 15; // Fixed rate per km
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="bg-gradient-to-r from-green-600 to-green-700 text-white">
        <CardContent className="p-6">
          <div className="flex items-center justify-between mb-6">
            <div>
              <p className="text-green-100 text-sm">Поточний період</p>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <Card>
          <CardContent className="p-4 text-center">
            ...
            <p className="text-sm text-gray-600">Кілометрів</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4 text-center">
            ...
            <p className="text-sm text-gray-600">₴/година</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4 text-center">
            <Wallet className="h-6 w-6 text-amber-600 mx-auto mb-2" />
            <p className="text-2xl font-bold text-gray-900">{kmRate}</p>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Деталізація по змінах</CardTitle>
        </CardHeader>
        <CardContent>
          {workerLogs.length === 0 ? (
            <div className="text-center py-8">
              <Clock3 className="h-12 w-12 text-gray-300 mx-auto mb-4" />
              <p className="text-gray-500">Ще немає даних про роботу</p>
            </div>
          ) : (
                ...
                <div key={log.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                  ...
                  <div className="text-right">
                    <p className="font-bold text-gray-900">
                      {((log.actual_hours * hourlyRate) + (log.km_driven * kmRate)).toLocaleString()} ₴
                    </p>
                    <p className="text-xs text-gray-500">
                      {log.actual_hours}×{hourlyRate} + {log.km_driven}×{kmRate}
                    </p>
                  </div>
                </div>
              ))}
        ...
        <CardContent>
          {workerSalaries.length === 0 ? (
            <div className="text-center py-8">
              ...
              <p className="text-gray-500">Ще немає виплат</p>
            </div>
          ) : (
            <div className="space-y-4">
                ...
                <div key={salary.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                  <div className="flex items-start space-x-4">
                    <div className={`p-2 rounded-lg ${salary.status === 'paid' ? 'bg-green-100' : 'bg-amber-100'}`}>
                      {salary.status === 'paid' ? (
                        <CheckCircle className={`h-5 w-5 ${salary.status === 'paid' ? 'text-green-600' : 'text-amber-600'}`} />
                      ) : (
                    ...
                    <div>
                      <p className="font-medium text-gray-900">
                        {new Date(salary.period_start).toLocaleDateString('uk-UA', { month: 'long' })}
                      </p>
                      <p className="text-sm text-gray-600">
                        {new Date(salary.period_start).toLocaleDateString('uk-UA')} - {new Date(salary.period_end).toLocaleDateString('uk-UA')}
                      </p>
                  ...
                  <div className="text-right">
                    <p className="text-xl font-bold text-gray-900">{salary.total_amount.toLocaleString()} ₴</p>
                    <Badge className={salary.status === 'paid' ? 'bg-green-600' : 'bg-amber-600'}>
                      {salary.status === 'paid' ? 'Виплачено' : 'Очікує'}
                    </Badge>
                  </div>
                </div>
              ))}
