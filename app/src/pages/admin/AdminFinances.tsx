import { mockOrders, mockWorkLogs, mockUsers } from '@/data/mockData';
...
export default function AdminFinances() {
  ...
  const workers = mockUsers.filter((u: { role: string }) => u.role === 'worker');
  
  // Calculate worker payments
  const workerPayments = workers.map(worker => {
    const workerLogs = mockWorkLogs.filter(l => l.worker_id === worker.id);
    const hours = workerLogs.reduce((sum, l) => sum + l.actual_hours, 0);
    const km = workerLogs.reduce((sum, l) => sum + l.km_driven, 0);
    const salary = hours * (worker.salary_hourly_rate || 0) + km * 15;
    return {
      ...worker,
      hours,
      km,
  ...
  const totalMaterials = mockOrders.reduce((sum, o) => sum + (o.materials_cost || 0), 0);
  const totalExpenses = totalWorkerPayments + totalMaterials;

  const profit = totalRevenue - totalExpenses;

  // Invoices
  const invoices = mockOrders.filter(o => o.total_amount);
    ...
    <div className="space-y-6">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card className="bg-gradient-to-br from-green-600 to-green-700 text-white">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-green-100 text-sm">Прибуток</p>
        ...
        <Card>
          <CardContent className="p-6">
            ...
            <div className="mt-2 flex items-center text-sm">
              <span className="text-green-600 font-medium">{paidRevenue.toLocaleString()} ₴</span>
              <span className="text-gray-500 ml-1">оплачено</span>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-6">
            ...
            <div className="mt-2 flex items-center text-sm">
              <span className="text-gray-600">Зарплата: {totalWorkerPayments.toLocaleString()} ₴</span>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-6">
            ...
            <div className="mt-2 flex items-center text-sm">
              <span className="text-gray-600">{pendingInvoices.length} рахунків</span>
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <Tabs defaultValue="invoices" className="space-y-6">
        ...
        <TabsContent value="invoices" className="space-y-6">
          <Card>
            <CardHeader>
              <CardTitle>Рахунки клієнтів</CardTitle>
            </CardHeader>
            <CardContent>
                        ...
                        <div>
                          <div className="flex items-center space-x-2">
                            ...
                            {invoice.status === 'paid' ? (
                              ...
                              <Badge variant="secondary" className="bg-amber-100 text-amber-700">Очікує</Badge>
                            )}
                          </div>
        ...
        <TabsContent value="workers" className="space-y-6">
          <Card>
            <CardHeader>
              <CardTitle>Зарплати працівників</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                    ...
                    <div className="flex items-start space-x-4">
                      ...
                      <div>
                        ...
                        <div className="flex items-center space-x-4 mt-1 text-sm text-gray-600">
                          ...
                          <span>{worker.km} км</span>
                          <span className="text-gray-500">{worker.salary_hourly_rate} ₴/год</span>
                        </div>
                      </div>
                    </div>
                    <div className="text-right">
                      ...
                      <p className="text-sm text-gray-500">
                        {worker.hours}×{worker.salary_hourly_rate} + {worker.km}×15
                      </p>
                    </div>
                  </div>
                ))}
        ...
        <TabsContent value="breakdown" className="space-y-6">
          <div className="grid md:grid-cols-2 gap-6">
            <Card>
              <CardHeader>
                <CardTitle>Структура доходів</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                  <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                    <span className="text-gray-600">Оплачено клієнтами</span>
                    <span className="font-bold text-green-600">{paidRevenue.toLocaleString()} ₴</span>
                  </div>
                  <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                    <span className="text-gray-600">Очікує оплати</span>
                    <span className="font-bold text-amber-600">{pendingRevenue.toLocaleString()} ₴</span>
                  </div>
                  <div className="flex justify-between items-center p-3 bg-green-50 rounded-lg border border-green-200">
                    <span className="font-semibold text-gray-900">Всього виручка</span>
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-4">
                  <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                    <span className="text-gray-600">Зарплата працівників</span>
                    <span className="font-bold text-red-600">{totalWorkerPayments.toLocaleString()} ₴</span>
                  </div>
                  <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                    <span className="text-gray-600">Матеріали</span>
                    <span className="font-bold text-red-600">{totalMaterials.toLocaleString()} ₴</span>
                  </div>
                  <div className="flex justify-between items-center p-3 bg-red-50 rounded-lg border border-red-200">
                    <span className="font-semibold text-gray-900">Всього витрати</span>
          ...
          <Card className="bg-gradient-to-r from-green-600 to-green-700 text-white">
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-green-100 text-sm">Чистий прибуток</p>
                  <p className="text-3xl font-bold">{profit.toLocaleString()} ₴</p>
                  <p className="text-green-200 text-sm mt-1">
                    {totalRevenue > 0 ? ((profit / totalRevenue) * 100).toFixed(1) : 0}% маржинальність
                  </p>
                </div>
